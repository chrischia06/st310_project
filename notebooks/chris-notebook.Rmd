...
Similar to Random Forests, the hyperparameters influence the *model complexity*. In this case we focus on two key hyperparameters: the maximum depth of each tree `max_depth`, and the `learning_rate`.  We keep the the number of trees or “boosting rounds” fixed `nround = 100` for faster training, although this could be a hyperparameter to tune as well.
...
We obtain a train accuracy of $0.766$ and a train AUC of $0.866$, versus a test accuracy of $0.753	$ and test AUC $0.871$. Interestingly, this suggests that the model may be underfitting for our particular choice of parameters (and also for our subsampled dataset size), and it is possible that further optimisations can be done.
...
## Conclusion
In this project, we demonstrated the use of logistic regression, gradient descent, regularisation, random forest, gradient boosting and hyperparameter tuning techniques to achieve interpretability and model accuracy (in separate cases) in predicting the amount of an insurance claim. From the models we explored, regularised logistic regression was the model with highest AUC, which suggests that a relatively interpretable model can also have a fair predictive performance. Nonetheless, in comparison with, our baseline model with only a few predictors, the regularised logistic regression is more difficult to interpret given the large number of predictors (when all levels of the categorical variable are considered). On the other hand, it is possible that a higher predictive performance could be attained by further fine-tuning a non-linear, less interpretable model, for example using a **deep neural network**. As a short remark, the winners of this contest made use of a *denoising auto-encoder* architecture (see [here](#https://www.kaggle.com/c/tabular-playground-series-mar-2021/discussion/229833). In practice, depending on whether the goals of the company lean towards prediction or explanation, an appropriate trade-off between model complexity and interpretability has to be chosen. 
